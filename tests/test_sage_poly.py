"""Tests for the Sage poly backend over F_p[x] (skipped if sage not on PATH)."""
...
# ---------------------------------------------------------------------------
# Helpers (same as test_pure_python_poly)
# ---------------------------------------------------------------------------
...
class TestSagePolySNFWithTransforms:
...
class TestSagePolyHNF:
    ...
    def test_matches_pure_python(self):
        for m_dict, p in [
            ...
            sage_r = poly_hermite_normal_form(m_dict, backend=BACKEND)
            pp_r = poly_hermite_normal_form(m_dict, backend="pure_python")
            assert _mat_eq(
                sage_r.hermite_normal_form.entries,
                pp_r.hermite_normal_form.entries,
            ), f"Sage HNF != pure_python HNF for p={p}"


class TestSagePolyHNFWithTransform:
    def _verify(self, matrix_dict: dict, p: int) -> None:
...
class TestSagePolyED:
    ...
    def test_matches_pure_python(self):
        for m_dict, p in [(_dense(_M2, 2), 2), (_dense(_DIAG3, 3), 3)]:
            sage_ed = poly_elementary_divisors(m_dict, backend=BACKEND)
            pp_ed = poly_elementary_divisors(m_dict, backend="pure_python")
            assert sage_ed.elementary_divisors == pp_ed.elementary_divisors


# ---------------------------------------------------------------------------
# Cross-backend consistency: sage vs pure_python (parametrized)
# ---------------------------------------------------------------------------
...
def test_sage_vs_pure_python_snf(m_dict, p, name):
    sage_r = poly_smith_normal_form(m_dict, backend="sage")
    pp_r = poly_smith_normal_form(m_dict, backend="pure_python")
    assert sage_r.invariant_factors == pp_r.invariant_factors, (
        f"SNF differs ({name}): sage={sage_r.invariant_factors} pp={pp_r.invariant_factors}"
    )


@pytest.mark.parametrize("m_dict,p,name", _CROSS_CASES, ids=[c[2] for c in _CROSS_CASES])
def test_sage_vs_pure_python_hnf(m_dict, p, name):
    sage_r = poly_hermite_normal_form(m_dict, backend="sage")
    pp_r = poly_hermite_normal_form(m_dict, backend="pure_python")
    assert _mat_eq(
        sage_r.hermite_normal_form.entries,
        pp_r.hermite_normal_form.entries,
