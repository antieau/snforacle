"""Tests for the SageMath finite-field backend and cross-backend consistency."""
...
class TestSageFFSNF:
    def test_identity_p5(self):
        result = ff_smith_normal_form(_dense(3, 3, 5, _identity(3)), backend="sage")
        assert result.rank == 3
        assert _is_snf(result.smith_normal_form.entries, 3, 3, 3)

    def test_zero_p7(self):
        result = ff_smith_normal_form(_dense(2, 2, 7, [[0, 0], [0, 0]]), backend="sage")
        assert result.rank == 0
        assert _is_snf(result.smith_normal_form.entries, 2, 2, 0)

    def test_rank1_p5(self):
        result = ff_smith_normal_form(_dense(2, 2, 5, [[1, 2], [2, 4]]), backend="sage")
        assert result.rank == 1
        assert _is_snf(result.smith_normal_form.entries, 2, 2, 1)

    def test_full_rank_3x3_p11(self):
        M = [[1, 2, 3], [4, 5, 6], [7, 8, 10]]
        result = ff_smith_normal_form(_dense(3, 3, 11, M), backend="sage")
        assert result.rank == 3
        assert _is_snf(result.smith_normal_form.entries, 3, 3, 3)

    def test_1x1_unit_p7(self):
        result = ff_smith_normal_form(_dense(1, 1, 7, [[3]]), backend="sage")
        assert result.rank == 1
        assert result.smith_normal_form.entries == [[1]]

    def test_nonsquare_2x3_p11(self):
        M = [[1, 0, 2], [0, 1, 3]]
        result = ff_smith_normal_form(_dense(2, 3, 11, M), backend="sage")
        assert result.rank == 2

    def test_return_type(self):
        result = ff_smith_normal_form(_dense(2, 2, 5, [[1, 2], [3, 4]]), backend="sage")
        assert isinstance(result, FFSNFResult)

    def test_p2(self):
        M = [[1, 1], [0, 1]]
        result = ff_smith_normal_form(_dense(2, 2, 2, M), backend="sage")
        assert result.rank == 2
...
class TestSageFFSNFWithTransforms:
    def _verify(self, M: list[list[int]], nrows: int, ncols: int, p: int) -> FFSNFWithTransformsResult:
        result = ff_smith_normal_form_with_transforms(
            _dense(nrows, ncols, p, M), backend="sage"
        )
    ...
    def test_return_type(self):
        result = ff_smith_normal_form_with_transforms(
            _dense(2, 2, 7, [[1, 2], [3, 4]]), backend="sage"
        )
...
class TestSageFFHNF:
    def test_identity_p5(self):
        result = ff_hermite_normal_form(_dense(3, 3, 5, _identity(3)), backend="sage")
        assert result.hermite_normal_form.entries == _identity(3)

    def test_zero_p7(self):
        result = ff_hermite_normal_form(_dense(2, 2, 7, [[0, 0], [0, 0]]), backend="sage")
        assert result.hermite_normal_form.entries == [[0, 0], [0, 0]]

    def test_full_rank_3x3_p11(self):
        M = [[1, 2, 3], [4, 5, 6], [7, 8, 10]]
        result = ff_hermite_normal_form(_dense(3, 3, 11, M), backend="sage")
        H = result.hermite_normal_form.entries
        assert _is_rref(H, 3, 3)
        assert H == _identity(3)

    def test_rank1_p5(self):
        result = ff_hermite_normal_form(_dense(2, 2, 5, [[1, 2], [2, 4]]), backend="sage")
        H = result.hermite_normal_form.entries
        assert _is_rref(H, 2, 2)

    def test_nonsquare_2x4_p7(self):
        M = [[1, 0, 2, 3], [0, 1, 4, 2]]
        result = ff_hermite_normal_form(_dense(2, 4, 7, M), backend="sage")
        H = result.hermite_normal_form.entries
        assert _is_rref(H, 2, 4)

    def test_return_type(self):
        result = ff_hermite_normal_form(_dense(2, 2, 5, [[1, 2], [3, 4]]), backend="sage")
...
class TestSageFFHNFWithTransform:
...
class TestSageFFRank:
    def test_full_rank(self):
        result = ff_rank(_dense(3, 3, 7, _identity(3)), backend="sage")
        assert result.rank == 3

    def test_zero(self):
        result = ff_rank(_dense(2, 2, 5, [[0, 0], [0, 0]]), backend="sage")
        assert result.rank == 0

    def test_rank1(self):
        result = ff_rank(_dense(2, 2, 5, [[1, 2], [2, 4]]), backend="sage")
        assert result.rank == 1

    def test_return_type(self):
        result = ff_rank(_dense(2, 2, 7, [[1, 0], [0, 1]]), backend="sage")
        assert isinstance(result, FFRankResult)


# ---------------------------------------------------------------------------
# Cross-backend consistency: sage vs pure_python
# ---------------------------------------------------------------------------

class TestSageFFVsPurePython:
    """Verify sage and pure_python backends agree on all operations."""
    ...
    def _check_snf(self, nrows, ncols, p, entries):
        ...
        r_sage = ff_smith_normal_form(inp, backend="sage")
        r_pp = ff_smith_normal_form(inp, backend="pure_python")
        assert r_sage.rank == r_pp.rank
        assert r_sage.smith_normal_form.entries == r_pp.smith_normal_form.entries

    def _check_hnf(self, nrows, ncols, p, entries):
        inp = self._inp(nrows, ncols, p, entries)
        r_sage = ff_hermite_normal_form(inp, backend="sage")
        r_pp = ff_hermite_normal_form(inp, backend="pure_python")
        assert r_sage.hermite_normal_form.entries == r_pp.hermite_normal_form.entries
